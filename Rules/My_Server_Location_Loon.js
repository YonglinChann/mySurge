...
//const paran = ["🌐 IP 地址","🎫 ASN 编号","ASN所属机构","💺 ISP 服务商","⌛️ 地区","🧩 城市","远端经度","远端纬度"];

const paras = ["query","as","isp","countryCode","city"];
const paran = ["🌐  IP 地址","🎫  ASN 编号","💺  ISP 服务商","⌛️  地区","🧩  城市"];

$httpClient.get(requestParams, (error, response, data) => {
	if (error) {
		message = "</br></br>⚠️ 查询超时"
		message = `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: bold;">` + message + `</p>`
		$done({"title": "🕹️ 节点信息查询", "htmlMessage": message});
	} else {
		console.log(data);
...
function json2info(cnt, paras) {
	//var res = "-------------------------------";
	var res = "\n\n";
	res = res + "</br>" + "<font color=#6959CD>" + "「 " + $environment.params.node + " 」\n\n" + "</font>";
	cnt = JSON.parse(cnt);
	console.log(cnt);
	for (i = 0;i < paras.length; i ++) {
		cnt[paras[i]] = paras[i] == "countryCode" ? cnt[paras[i]] + " " + flags.get(cnt[paras[i]].toUpperCase()) : cnt[paras[i]];
		res = cnt[paras[i]] ? res + "</br><b>" + "<font  color=>" + paran[i] + "</font> 👇🏻\n" + "</b>"+ "<font  color=>" + cnt[paras[i]] + "</font></br>" : res;
	}
